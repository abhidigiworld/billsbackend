...
exports.submitAttendance = async (req, res, next) => {
  try {
    ...
    const approvalDocs = records.map(rec => ({
      ...
      checkIn: rec.status === 'Present' && rec.workedDay && rec.checkIn ? new Date(`${date}T${rec.checkIn}:00`) : null,
      checkOut: rec.status === 'Present' && rec.workedDay && rec.checkOut ? new Date(`${date}T${rec.checkOut}:00`) : null,
      nightCheckIn: rec.status === 'Present' && rec.workedNight && rec.nightCheckIn ? new Date(`${date}T${rec.nightCheckIn}:00`) : null,
      nightCheckOut: rec.status === 'Present' && rec.workedNight && rec.nightCheckOut ? new Date(`${date}T${rec.nightCheckOut}:00`) : null,
